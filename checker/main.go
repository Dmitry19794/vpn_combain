...
func main() {
    ...
    log.Printf("🚀 Starting VPN Checker v1.0")
    log.Printf("📍 GEO: %s", config.Geo)
    log.Printf("👷 Workers: %d-%d", config.MinWorkers, config.MaxWorkers)
    // Подключение к БД
    ...
    // Создаём и запускаем checker — функция NewChecker видна напрямую из checker.go (package main)
    checker := NewChecker(db, config.Geo, config.MinWorkers, config.MaxWorkers)
    ...
    select {
    case <-done:
        log.Printf("✅ Checker stopped gracefully")
...
func parseFlags() *Config {
    ...
    flag.StringVar(&config.DBPassword, "db-pass", getEnv("DB_PASS", "your_password_here"), "Database password")
    ...
    flag.IntVar(&config.MinWorkers, "min-workers", 10, "Minimum workers")
    flag.IntVar(&config.MaxWorkers, "max-workers", 100, "Maximum workers")
